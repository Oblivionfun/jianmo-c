...
base_sens = sens.loc[sens["pv_factor"].sub(0.9).abs() < 1e-9].iloc[0]
row_08 = sens.loc[sens["pv_factor"].sub(0.8).abs() < 1e-9].iloc[0]
row_10 = sens.loc[sens["pv_factor"].sub(1.0).abs() < 1e-9].iloc[0]
p(
    "敏感性实验只改变光伏预测的保守系数，负荷预测、价格序列、储能边界、调度模型和结算规则保持不变。"
    "这样可以把‘光伏预测偏差’从其他因素中单独抽出：系数低于1表示计划侧少估光伏，系数等于1表示直接采用因果中位数预测。"
    "比较时同时记录全年费用和紧急购电量，因为少购计划电可能降低紧急缺口，却会把成本转移到计划购电；单看其中一个指标会误判。"
)
...
p(
    f"以0.9为基线，0.8使全年费用增加{row_08.q2_cost-base_sens.q2_cost:.2f}元（{100*(row_08.q2_cost/base_sens.q2_cost-1):.2f}%），"
    f"但紧急购电量减少{base_sens.q2_emergency_kwh-row_08.q2_emergency_kwh:.3f} kWh（{100*(1-row_08.q2_emergency_kwh/base_sens.q2_emergency_kwh):.2f}%）；"
    f"1.0使费用增加{row_10.q2_cost-base_sens.q2_cost:.2f}元（{100*(row_10.q2_cost/base_sens.q2_cost-1):.2f}%），紧急购电量增加{row_10.q2_emergency_kwh-base_sens.q2_emergency_kwh:.3f} kWh（{100*(row_10.q2_emergency_kwh/base_sens.q2_emergency_kwh-1):.2f}%）。"
    "因此费用曲线不是由紧急电量单调决定的，而是计划购电、储能机会成本和紧急结算共同作用的结果。"
    "0.9在当前回测窗口给出最低费用，但这个选择只对当前附件、预测窗口和成本口径成立，不宣称为普适最优。"
)
h("8.2 对抗性核验")
p(
    "稳健性核验先检查账单恒等式，再检查分支之间应保持或应改变的量。对Q3和Q4-3，全年账单分别按‘计划基准费+调减费+调增费+紧急费’逐日相加；"
    "对Q4-2，策略沿用Q2的计划，只替换价格序列，因此调度量和紧急购电量应与Q2一致。外部经验贴仅用于方法讨论，本文不采用其中未经复现的数字或未来信息路径。"
)
p(
    f"当前结果满足上述边界：Q4-2与Q2的全年紧急购电量差异小于10^-9 kWh，说明价格替换没有偷偷改变调度；"
    f"Q4-3相对Q3的全年费用增加{daily['q4_3_cost'].sum()-daily['q3_cost'].sum():.2f}元（{100*(daily['q4_3_cost'].sum()/daily['q3_cost'].sum()-1):.2f}%），"
    f"而紧急购电量仅增加{daily['q4_3_emergency_kwh'].sum()-daily['q3_emergency_kwh'].sum():.3f} kWh（{100*(daily['q4_3_emergency_kwh'].sum()/daily['q3_emergency_kwh'].sum()-1):.2f}%）。"
    "这表明动态价格下的差异主要来自结算价格和调增/调减结构，而不能被简单归因于缺口数量。"
)
p(
    "信息边界也按代码路径复核：目标日的负荷和光伏预测只从目标日前观测构造，实际负荷与实际光伏只在事后紧急结算中使用；"
    "Q1的MILP与5 kWh网格DP使用同一日初、日末储能条件，结果工作簿保留四问所需工作表和逐十分钟轨迹。"
)
h("8.3 局限")
p(
    "当前模型把预测表示为单一路径，光伏系数实验只改变幅值，不能覆盖误差的时间相关性、负荷—光伏联合误差和极端天气场景；"
    "同时尚未建立完整场景树或概率预测区间，也未建模通信延迟、电池退化、负荷可移峰和CVaR风险度量。"
    "因此敏感性表回答的是‘在当前回测窗口内改变一个参数会怎样’，不能替代概率稳健性结论。"
)
p(
    "回测覆盖一个附件年度的334天，且采用固定随机种子和单一路径预测；这足以支持本地复现和方法比较，不能外推到其他年份或真实运行系统。"
    "若后续引入鲁棒或随机模型，应先明确场景生成和风险预算，再重算全部结果、重新生成图表并复核账单恒等式。"
)
fig("process_q2_q4_compare", "图8  固定价格与波动价格下的日费用对比", width=4.2)
p(
    "图8只承担价格场景的总体趋势展示，具体数值以表8、表9和逐日结果文件为准。图中尖峰不能单独解释为价格原因，"
    "还需要结合同日的负荷、光伏、储能状态和调整量共同判断；这也是本文同时保留分项账单和紧急购电指标的原因。"
)

H("九、结论与可复现性")
p(
    "本文以统一的十分钟能量平衡模型贯穿四个问题，但每问保留自己的信息边界和评价口径。问题一用互斥MILP得到可执行的单日基准，"
    "5 kWh网格DP在相同日初、日末储能条件下独立校验库存价值；问题二用日期因果预测完成全年计划和实际缺口结算；问题三把新预报转化为锁定前缀后的滚动再决策，"
    "并按题面规则拆解计划、调减、调增和紧急费用；问题四从问题二和问题三分别启动独立价格分支，避免把价格替换误写成调度改进。"
)
p(
    f"四问的主要结果可以直接对应题面任务：问题一的MILP目标值为{summary['q1']['objective']:.2f}元，网格DP与其相对差{summary['q1']['dp_relative_gap_pct']:.4f}%；"
    f"问题二在334天回测中的费用和紧急购电量分别为{agg['q2']['cost']:.2f}元、{agg['q2']['emergency_kwh']:.3f} kWh；"
    f"问题三对应为{agg['q3']['cost']:.2f}元、{agg['q3']['emergency_kwh']:.3f} kWh。相对Q2，滚动调整使费用增加{agg['q3']['cost']-agg['q2']['cost']:.2f}元（{100*(agg['q3']['cost']/agg['q2']['cost']-1):.2f}%），"
    f"紧急购电量增加{agg['q3']['emergency_kwh']-agg['q2']['emergency_kwh']:.3f} kWh（{100*(agg['q3']['emergency_kwh']/agg['q2']['emergency_kwh']-1):.2f}%）；这个差异反映了新预报触发的再决策和结算代价，不能直接解释为单一算法优劣。"
)
p(
    f"问题四的固定计划价格替换分支Q4-2费用为{agg['q4']['cost']:.2f}元，与Q2的紧急购电量差异小于10^-9 kWh；"
    f"波动价格滚动分支Q4-3费用为{agg['q4_3']['cost']:.2f}元，比Q3高{agg['q4_3']['cost']-agg['q3']['cost']:.2f}元（{100*(agg['q4_3']['cost']/agg['q3']['cost']-1):.2f}%），"
    f"但紧急购电量只高{agg['q4_3']['emergency_kwh']-agg['q3']['emergency_kwh']:.3f} kWh（{100*(agg['q4_3']['emergency_kwh']/agg['q3']['emergency_kwh']-1):.2f}%）。"
    "因此波动价格的主要影响落在价格暴露和调增/调减结算结构，而非缺口数量本身。"
)
p(
    "复现时先运行输入审计，再执行固定种子的求解脚本；随后用结果校验脚本检查汇总量、工作簿和图形，用论文校验脚本检查公式、图表和页数。"
    "输入哈希、Python版本、依赖版本、模型参数和输出清单统一写入复现记录，便于把‘程序能运行’与‘结果可追溯’区分开。"
)
...
compact_table([["交付物", "位置", "用途"], ["求解脚本", "solve_c.py", "重算Q1—Q4"], ["结果工作簿", "results/result*.xlsx", "按题面模板复核和提交准备"], ["论文Word", "完整论文.docx", "可编辑正文和公式"], ["论文LaTeX", "完整论文-LaTeX/main.tex", "Overleaf编译"], ["图形", "figures/*.png,*.svg", "论文插图和编辑"], ["研究记录", "research/", "样本检索、经验贴审查与方法边界"]], font_size=8.5)
