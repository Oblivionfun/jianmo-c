...
def inspect(path):
  with ZipFile(path) as z:
    ...
    for sh in wb.find(f'{{{NS}}}sheets'):
      ...
      root=ET.fromstring(z.read(target)); rows=root.findall(f'.//{{{NS}}}sheetData/{{{NS}}}row')
      vals=[]
      for row in rows:
        for c in row.findall(f'{{{NS}}}c'):
          v=textval(c,shared)
          if v is not None and v!='': vals.append(v)
      out.append({'sheet':sh.attrib['name'],'rows':len(rows),'nonempty_cells':len(vals)})
    return out
